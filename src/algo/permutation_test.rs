...
fn run_algorithm_on_permutation<'a, T, B>(detector: &B, true_statistic: &T, permutation: &[T]) -> Result<f64>
    where
    T: Ord + Clone,
...
fn do_permutation_iteration<T, B>(algorithm: &B, true_statistic: &T, permutation_iterations: &mut [PermutationIteration<T>]) -> ()
    where
    ...
    B: ChangePointDetector<T> + Send + Sync,
{
    if permutation_iterations.len() <= 1 {
        for permutation_iteration in permutation_iterations {
            permutation_iteration.greater_than_truth = Some(run_algorithm_on_permutation(algorithm, true_statistic, &permutation_iteration.permutation));
        };
    } else {
        let slice_point: usize = permutation_iterations.len() / 2;
        let (left, right) = permutation_iterations.split_at_mut(slice_point);
        rayon::join(|| do_permutation_iteration(algorithm, true_statistic, left), || do_permutation_iteration(algorithm, true_statistic, right));
    }
}

pub fn permutation_test<'a, T, B, R>(algorithm: &B, mut rng: R, num_permutations: usize, observations: &'a [T]) -> Result<PermutationTestResult>
    where
    T: Ord + Clone + Send + Sync,
    B: ChangePointDetector<T> + Send + Sync,
    R: Rng,
{
    let BestCandidate { statistic: true_statistic, location: true_location } = algorithm.find_candidate(observations)?;
    ...
    let num_failures: Result<f64> = permutations.into_iter()
        .fold(Ok(0.0), |num_failures, permutation| {
            Ok(num_failures? + match permutation.greater_than_truth {
                Some(result) => result?,
